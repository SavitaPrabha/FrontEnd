...
function SpecialCollection(props) {
  const { products = [], dealsAndPromotions=[], rated = [], loading } = props;
      ...
      <div className="container">
        <TabList className="nav nav-pills nav-border-anim nav-big justify-content-center mb-3">
          <Tab className="nav-item">
            <span className="nav-link">Deals & Promotions</span>
          </Tab>
        ...
        <TabPanel>
          {loading ? (
            ...
            <OwlCarousel
              ...
              options={productSlider}
            >
              {attrFilter(dealsAndPromotions, "all").map((item, index) => (
        ...
        <TabPanel>
          {loading ? (
            ...
            <OwlCarousel
              ...
              options={productSlider}
            >
              {attrFilter(rated, "all").map((item, index) => (
                <ProductTwelve product={item} key={index} />
              ))}
            </OwlCarousel>
          )}
